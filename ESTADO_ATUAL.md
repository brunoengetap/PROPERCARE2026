...
**Última atualização:** 06/07/2026 · **Por:** **Auditoria completa do sistema** (todos os 5 frontends + GAS) + Sprints **FixDataCSS** (PCM v64→v65) e **FixBuscaFoco** (PCM v65→**v66**). Lacuna de changelog **PCM v62/v63/v64 preenchida** em §2 a partir do changelog do próprio HTML (auditado nesta sessão). · Entregáveis desta sessão: **PCM v66** (aplicado + validado por Claude neste ambiente: `node --check` real + parser HTML + greps de invariantes).
...
| PCF (campo) | `PCF_index_v43.html` | ✅ auditado (FormsPipeline) — **`node --check` real** nos 2 blocos `<script>` (passam). FIX 2b: fallback de `pfGetFormUrl` para `auto:form08` aponta agora para `form08.html` (0 refs à versão antiga, 2 à nova). Handoff confirmado: `pfAbrirForm` anexa `os_id`/`tarefa_id` à URL antes de navegar. ⚠️ Sprints v40→v42 **sem changelog em §2** (ver aviso acima). **Ainda não publicado** |
