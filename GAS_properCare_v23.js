  ...
  try {
    ...
    switch (action) {
      ...
      case 'saveSubsistemaLib':
        result = saveSubsistemaLib(body.data || body.subsistema || {});
        break;
      case 'deleteSubsistemaLib':
        result = deleteSubsistemaLib(body.subLibId, body.deletedBy || 'admin');
        break;
      case 'saveSubLibPart':
        result = saveSubLibPart(body.data || body.part || {});
        break;
...
function getCatalogFull() {
  ...
  return {
    ...
    modeloSubLinks: getSheetDataActive('MODELO_SUBSISTEMA_LIB_LINKS')
  };
}
...
function getSubsistemasLibFull() {
  ...
  ensureSheetHeaders('SUBSISTEMA_LIB_PARTS', HEADERS.SUBSISTEMA_LIB_PARTS);
  ensureSheetHeaders('MODELO_SUBSISTEMA_LIB_LINKS', HEADERS.MODELO_SUBSISTEMA_LIB_LINKS);
  return { status: 'ok', subsistemasLib: getSheetDataActive('SUBSISTEMAS_LIB'), subLibParts: getSheetDataActive('SUBSISTEMA_LIB_PARTS'), modeloSubLinks: getSheetDataActive('MODELO_SUBSISTEMA_LIB_LINKS') };
}

function _findRowByHeader_(sheet, idHeader, id) {
  const data = sheet.getDataRange().getValues();
  const headers = data[0] || [];
  const idx = headers.indexOf(idHeader);
  for (let i = 1; i < data.length; i++) if (String(data[i][idx]) === String(id)) return { rowIdx: i, headers: headers, data: data };
  return { rowIdx: -1, headers: headers, data: data };
}

function saveSubsistemaLib(data) {
  const lock = LockService.getScriptLock(); lock.waitLock(10000);
  try {
    const sheet = getOrCreateSheet('SUBSISTEMAS_LIB', HEADERS.SUBSISTEMAS_LIB); ensureSheetHeaders('SUBSISTEMAS_LIB', HEADERS.SUBSISTEMAS_LIB);
    const now = new Date().toISOString(); const id = data.id || data.subLibId || Utilities.getUuid();
    const found = _findRowByHeader_(sheet, 'Sub_Lib_ID', id); const headers = found.headers;
    const rowData = { Sub_Lib_ID:id, Nome:data.nome||data.name||'', Categoria:data.categoria||data.category||'', Descricao:data.descricao||data.desc||'', Interval_Base_H:parseInt(data.intervalBaseH||data.interval||0,10)||0, Codigo_Referencia:data.codigoReferencia||data.codigo||'', Fabricante:data.fabricante||data.manufacturer||'', Obs:data.obs||'', Ativo:'SIM', Tipo_Registro:data.tipoRegistro||'PRODUCAO', Updated_At:now };
    if (found.rowIdx < 0) { rowData.Created_At=now; rowData.Created_By=data.createdBy||'admin'; sheet.appendRow(headers.map(h => rowData[h] !== undefined ? rowData[h] : '')); }
    else headers.forEach((h,c)=>{ if(rowData[h]!==undefined) sheet.getRange(found.rowIdx+1,c+1).setValue(rowData[h]); });
...
function deleteSubsistemaLib(subLibId, deletedBy) {
  const lock = LockService.getScriptLock(); lock.waitLock(10000);
  try {
    const linksSheet=getOrCreateSheet('MODELO_SUBSISTEMA_LIB_LINKS',HEADERS.MODELO_SUBSISTEMA_LIB_LINKS); ensureSheetHeaders('MODELO_SUBSISTEMA_LIB_LINKS',HEADERS.MODELO_SUBSISTEMA_LIB_LINKS);
    const linkRows=linksSheet.getDataRange().getValues(); const lh=linkRows[0]||[];
    const hasActiveLink=linkRows.slice(1).some(r=>String(r[lh.indexOf('Sub_Lib_ID')])===String(subLibId)&&String(r[lh.indexOf('Ativo')]).toUpperCase()==='SIM');
    if(hasActiveLink) return {status:'error',error:'Subsistema vinculado a pelo menos um modelo. Desvincule antes de excluir.'};
    return _softDeleteById_('SUBSISTEMAS_LIB','Sub_Lib_ID',subLibId,deletedBy||'admin','Subsistema não encontrado');
  } finally { lock.releaseLock(); }
}
...
function saveSubLibPart(data) {
  const lock = LockService.getScriptLock(); lock.waitLock(10000);
  try {
    const sheet=getOrCreateSheet('SUBSISTEMA_LIB_PARTS',HEADERS.SUBSISTEMA_LIB_PARTS); ensureSheetHeaders('SUBSISTEMA_LIB_PARTS',HEADERS.SUBSISTEMA_LIB_PARTS);
    const now=new Date().toISOString(); const id=data.id||data.partId||Utilities.getUuid(); const found=_findRowByHeader_(sheet,'Part_ID',id); const headers=found.headers;
    const rowData={Part_ID:id,Sub_Lib_ID:data.subLibId||'',Name:data.name||'',OEM_Ref:data.oemRef||data.ref||'',Part_Brand:data.partBrand||'',Supplier_Primary:data.supplierPrimary||data.supplier||'',Slot:data.slot||'',Qty_Default:parseInt(data.qtyDefault||data.qty||1,10)||1,Interval_H:parseInt(data.intervalH||data.interval||0,10)||0,Criticality:data.criticality||'normal',Cost:parseFloat(data.cost)||0,Obs:data.obs||'',Ativo:'SIM',Tipo_Registro:data.tipoRegistro||'PRODUCAO',Updated_At:now};
    if(found.rowIdx<0){rowData.Created_At=now; sheet.appendRow(headers.map(h=>rowData[h]!==undefined?rowData[h]:''));}
    else headers.forEach((h,c)=>{if(rowData[h]!==undefined) sheet.getRange(found.rowIdx+1,c+1).setValue(rowData[h]);});
    return {status:'ok',id:id};
  } finally { lock.releaseLock(); }
}

function deleteSubLibPart(partId, deletedBy) { const lock=LockService.getScriptLock(); lock.waitLock(10000); try { return _softDeleteById_('SUBSISTEMA_LIB_PARTS','Part_ID',partId,deletedBy||'admin','Peça não encontrada'); } finally { lock.releaseLock(); } }

function _softDeleteById_(sheetName, idHeader, id, deletedBy, notFoundMsg) {
  const sheet=getOrCreateSheet(sheetName,HEADERS[sheetName]); ensureSheetHeaders(sheetName,HEADERS[sheetName]); const data=sheet.getDataRange().getValues(); const h=data[0]||[]; const idx=h.indexOf(idHeader); const now=new Date().toISOString();
  for(let i=1;i<data.length;i++) if(String(data[i][idx])===String(id)){ if(h.indexOf('Ativo')>=0) sheet.getRange(i+1,h.indexOf('Ativo')+1).setValue('NÃO'); if(h.indexOf('Deleted_At')>=0) sheet.getRange(i+1,h.indexOf('Deleted_At')+1).setValue(now); if(h.indexOf('Deleted_By')>=0) sheet.getRange(i+1,h.indexOf('Deleted_By')+1).setValue(deletedBy||'admin'); if(h.indexOf('Updated_At')>=0) sheet.getRange(i+1,h.indexOf('Updated_At')+1).setValue(now); return {status:'ok'}; }
  return {status:'error',error:notFoundMsg};
}

function linkSubsistemaToModel(modelId, subLibId, createdBy) {
  const lock=LockService.getScriptLock(); lock.waitLock(10000);
  try { const sheet=getOrCreateSheet('MODELO_SUBSISTEMA_LIB_LINKS',HEADERS.MODELO_SUBSISTEMA_LIB_LINKS); ensureSheetHeaders('MODELO_SUBSISTEMA_LIB_LINKS',HEADERS.MODELO_SUBSISTEMA_LIB_LINKS); const data=sheet.getDataRange().getValues(); const h=data[0]||[]; const dup=data.slice(1).some(r=>String(r[h.indexOf('Model_ID')])===String(modelId)&&String(r[h.indexOf('Sub_Lib_ID')])===String(subLibId)&&String(r[h.indexOf('Ativo')]).toUpperCase()==='SIM'); if(dup) return {status:'error',error:'Este subsistema já está vinculado a este modelo.'}; const id=Utilities.getUuid(); const row={Link_ID:id,Model_ID:modelId,Sub_Lib_ID:subLibId,Ativo:'SIM',Created_At:new Date().toISOString(),Created_By:createdBy||'admin'}; sheet.appendRow(h.map(x=>row[x]!==undefined?row[x]:'')); return {status:'ok',id:id}; } finally { lock.releaseLock(); }
}

function unlinkSubsistemaFromModel(modelId, subLibId, deletedBy) {
  const lock=LockService.getScriptLock(); lock.waitLock(10000);
  try { const sheet=getOrCreateSheet('MODELO_SUBSISTEMA_LIB_LINKS',HEADERS.MODELO_SUBSISTEMA_LIB_LINKS); ensureSheetHeaders('MODELO_SUBSISTEMA_LIB_LINKS',HEADERS.MODELO_SUBSISTEMA_LIB_LINKS); const data=sheet.getDataRange().getValues(); const h=data[0]||[]; const now=new Date().toISOString(); for(let i=1;i<data.length;i++){ if(String(data[i][h.indexOf('Model_ID')])===String(modelId)&&String(data[i][h.indexOf('Sub_Lib_ID')])===String(subLibId)&&String(data[i][h.indexOf('Ativo')]).toUpperCase()==='SIM'){ sheet.getRange(i+1,h.indexOf('Ativo')+1).setValue('NÃO'); sheet.getRange(i+1,h.indexOf('Deleted_At')+1).setValue(now); sheet.getRange(i+1,h.indexOf('Deleted_By')+1).setValue(deletedBy||'admin'); return {status:'ok'}; }} return {status:'error',error:'Vínculo não encontrado'}; } finally { lock.releaseLock(); }
}
...
function initializeDatabase() {
  ...
  const SHEET_DEFS = [
    ...
    { name: 'MODELO_SUBSISTEMA_LIB_LINKS', headers: HEADERS.MODELO_SUBSISTEMA_LIB_LINKS },
  ];
