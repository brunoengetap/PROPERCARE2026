// v56 sprint: PCF/OS/preventiva updates.
// ============================================================
...
//     getSheetDataActive('MAQUINAS') (exclui Ativo=NÃO). Uma máquina vinculada
//     a uma OS e depois soft-deletada deixava de resolver, exibindo a chave crua
//     "MK-..." como "Máquina não encontrada" — mesmo com a chave batendo 100%.
//   - Correção cirúrgica (só getMaquinasByOS): o mapa passa a usar
//     getSheetData('MAQUINAS') (todas as linhas), com precedência da ativa sobre
//     a inativa quando compartilham chave/ID. A máquina resolvida ganha o campo
//     inativa:true|false para a UI poder marcá-la. Não altera OS_MAQUINAS nem
//     reativa nada — apenas exibe o vínculo histórico corretamente.
//
// GAS_properCare_v53.js
// v53 — Fix pipeline: campos por tarefa (Usuario_Designado, Data_Prevista,
//        Tempo_Efetivo_Min, status via registrarAbertura) não persistiam na
//        visão quando a OS já tinha tido pipeline trocado/reinstanciado.
//   - Causa: instanciarPipeline arquiva o pipeline anterior (Ativo='ARQUIVADO')
//     mantendo o MESMO Tarefa_ID; as 4 funções de escrita casavam a linha só por
//     OS_ID+Tarefa_ID e gravavam na PRIMEIRA (a arquivada, que vem antes na aba),
//     enquanto getPipelineByOS só retorna as ATIVAS → gravava numa linha, exibia
//     outra (vazia).
//   - Correção cirúrgica: as 4 funções agora pulam linhas ARQUIVADO/NÃO no laço
//     de match, escrevendo somente na linha ativa (mesmo conjunto que
//     getPipelineByOS enxerga): updateTarefaStatus, designarUsuarioTarefa,
//     registrarAbertura, ajustarTempoEfetivoTarefa.
//   - Sem mudança de assinatura, de header ou de roteamento.
//
// GAS_properCare_v52.js
// v52 — Fix roteamento doGet/doPost (bugs de "Ação desconhecida"):
//   - doPost: +getMaquinasByOS, +getEquipamentosByOS (chamados por POST no PCM
//     via syncToGS — vínculo de máquina na OS não carregava; card de detalhe e
//     pré-carregamento do seletor no openModalOS caíam no default).
//   - doPost: +getCatalogFull (chamado por POST no PCF via pgpPostJson —
//     catálogo do app de campo não carregava).
//   - Sem mudança de assinatura; getMaquinasByOS(body) lê body.id_os/numero_os.
//   - doGet inalterado — compatibilidade dupla GET+POST mantida (mesmo padrão de
//     getUsuarios/getTecnicos, getPipelineByOS, getTemplates, getDraftOS).
//
// GAS_properCare_v51.js
// v51 — Fix persistência PCF↔GAS: Tipo de Equipamento, Regime de trabalho e
//        horímetros por peça (valor mostrado/contador) não sobreviviam ao
//        recarregar a máquina no PCF, especialmente vindos de preventiva.
//   - HEADERS.MAQUINAS: nova coluna Regime_JSON ao final (retrocompatível,
//     mesmo padrão de Potência/Tipo_Equip/Obs_Op da v1.9). Guarda o estado
//     ATUAL do regime de trabalho (última visita que o enviou) — não
//     confundir com VISITAS.Regime_JSON, que é histórico por visita.
//   - ensureMachineFromVisit: passa a ler body.regimeData e gravar em
//     MAQUINAS.Regime_JSON, seguindo o mesmo padrão condicional já usado
//     para potencia/tipoEquip/obsOp (só grava se veio valor).
//   - rowToMachine: novos aliases tipo/potencia/regimeData (mantidos
//     type/power por retrocompatibilidade) — são os nomes que o PCF
//     (pgpLoadMachine) realmente lê ao restaurar a tela.
//   - updateMachinePartValorMostrado: agora faz upsert (criava Logger.log e
//     descartava o dado quando a linha não existia em MACHINE_PARTS — ex.
//     peça "conferida"/"N/A" numa máquina sem peças cadastradas ainda).
//     Assinatura ganhou parâmetro extra {name,na,interval,lastChange} para
//     poder criar a linha nova; chamada em savePreventiva atualizada.
//   Base: parte do v50 abaixo (fix de OS_MAQUINAS), que permanece intacto.
// GAS_properCare_v50.js
// v50 — Fix: saveOS não sincronizava OS_MAQUINAS na edição de OS
//   - Bug: no branch de edição de saveOS(), quando body.maquinas_vinculadas era
//     enviado, o código só regravava a string serializada no campo
//     maquinas_vinculadas da própria linha de ORDENS_SERVICO. Só o branch de
//     criação chamava _upsertOsMaquina_. Como getMaquinasByOS() (usado pelo PCM
//     para pré-marcar checkboxes ao reabrir "Editar OS" e pelo PCF em
//     pgpIniciarPreventivaDeOS) lê exclusivamente de OS_MAQUINAS, editar uma OS
//     depois de criada não refletia máquinas adicionadas/removidas na edição —
//     o PCF continuava enxergando o estado da criação.
//   - Fix: branch de edição agora reconcilia OS_MAQUINAS com o novo conjunto de
//     maquinas_vinculadas logo após calcular osAtualizada — vincula/reativa (via
//     _upsertOsMaquina_, reaproveitada sem alterações) as máquinas novas e
//     desativa (Ativo='NÃO', Updated_At) as que saíram da seleção, no mesmo
//     padrão usado por vincularMaquinaOS no modo 'desvincular'. Roda dentro de
//     try/catch não-bloqueante: falha na reconciliação é logada (Logger.log) mas
//     nunca reverte a gravação da OS já feita (mesmo padrão de
//     _marcarMaquinaAtendidaOS_). Branch de criação não foi tocado.
...
// GAS_properCare_v48.js
// v48 — Finaliza atendimento de OS ao salvar preventiva vinculada
//   - Nova função _finalizarAtendimentoOS_: ao registrar preventiva com body.os_id,
//     grava fim_atendimento na OS sem passar por saveOS/FECHAMENTO_INCOMPLETO.
//   - Se houver tarefas em PIPELINE_TAREFAS para a OS e todas estiverem com Status=completo,
//     atualiza o status da OS para concluida; se não houver tarefas, só grava fim_atendimento.
//   - savePreventiva: chama a finalização no bloco body.os_id após autoCompletarTarefa, junto
//     das integrações existentes de OS_MAQUINAS e visit_ids_resultado_json.
// GAS_properCare_v47.js
// v47 — Reconecta resolução de módulos/is_admin no loginTecnico
//   - Motivo: desde o rollback v44→v46 (loginTecnico substituindo loginUsuarioHub),
//     o login por PIN passou a retornar só { tecnico: { id, nome, perfil } } — sem
//     modulos/is_admin. O renderMenu() do ProperHub_index depende desses dois campos
//     para liberar os cards do menu; sem eles, todo login funcionava mas o menu
//     inteiro ficava bloqueado.
//   - Nova função resolverModulosUsuario_(idUsuario): extrai (sem alterar a regra)
//     a lógica de cruzamento CAT_USUARIOS × CAT_PERFIS que já existia embutida em
//     loginUsuarioHub — modulos_acesso do perfil + acesso_extra do usuário =
//     modulos_efetivos; is_admin do perfil libera todos os módulos. Retorna
//     { modulos: [], is_admin: false } se o usuário ou o perfil não forem
//     encontrados (login continua válido, só sem módulos liberados).
//   - loginTecnico: chama resolverModulosUsuario_(tec.id) e inclui modulos/is_admin
//     no objeto tecnico retornado.
//   - loginUsuarioHub (código morto, mantido para rollback): não alterada — decisão
//     consciente de não mexer em código não roteado para reduzir risco; a lógica
//     que ela já tinha embutida é idêntica à extraída para resolverModulosUsuario_.
// GAS_properCare_v46.js
// v46 — Rollback: remoção da camada de acesso Google (domínio), volta ao PIN simples
//   - Motivo: a camada OAuth por domínio (propercompressores.com.br), introduzida
//     na v28 e refinada até a v44, estava consumindo tempo demais para estabilizar
//     (lentidão, falhas de conexão intermitentes). Uma tentativa de corrigir
//     performance via JWKS local não é viável no Apps Script (BigInt não é
//     suportado de forma confiável nesse runtime — ver v45, nunca implantada).
//     O PCF é prioridade agora; o acesso ao ProperHub volta a ser só por PIN.
//   - doGet: removida a checagem verificarGoogleToken_(params.google_credential)
//     e o try/catch de 'AUTH_GOOGLE' em volta dela. Mantido apenas checkKey(params).
//   - doPost: removido o bloco que chamava verificarGoogleToken_(body.google_credential)
//     para ações diferentes de loginTecnicoHub/loginUsuarioHub. Mantido apenas
//     checkKey(body). Removido também o case 'loginUsuarioHub'/'loginTecnicoHub'
//     do switch (essas ações não são mais roteáveis por doPost).
//   - verificarGoogleToken_, loginUsuarioHub, loginTecnicoHub e as constantes
//     GOOGLE_CLIENT_ID_HUB/GOOGLE_DOMAIN_HUB foram mantidas no arquivo como
//     código morto (não referenciado) — não foram apagadas para manter o
//     rollback reversível caso a camada Google seja retomada no futuro.
//   - Nova função verificarPinsDuplicados_(): lê CAT_USUARIOS ativos e aponta
//     grupos de usuários que compartilham o mesmo pin_hash (login por PIN
//     simples fica ambíguo em caso de duplicidade — "vence" o primeiro match
//     em loginTecnico). Exposta em doGet via ?action=verificarPinsDuplicados
//     (exige só a key de sempre, sem checagem adicional).
// GAS_properCare_v44.js
// v44 — Sprint: Segurança do login (ProperHub) + sessão resiliente a baixa conectividade
//   - verificarGoogleToken_: agora valida a assinatura do JWT chamando o
//     endpoint oficial do Google (tokeninfo) em vez de só decodificar o
//     payload sem checar assinatura. Antes, qualquer string no formato de
//     JWT com aud/hd/exp "corretos" passava pela checagem — sem nunca ter
//     havido login real com uma conta Google.
//   - loginUsuarioHub (Modo B): rate-limit de tentativas de PIN via
//     CacheService — 5 tentativas erradas bloqueiam novas tentativas por
//     5 minutos (chave por id_usuario). Antes não havia limite nenhum.
// GAS_properCare_v43.js
// v43 — Sprint: Preventiva↔Pipeline de OS (integração PCF↔ProperFlow)
//   - getMaquinasByOS: máquinas retornadas agora normalizadas via rowToMachine() + parts
//     (getMachinePartsById), no mesmo formato que getMachinesByClient já usa — permite o
//     PCF reaproveitar pgpLoadMachine/pgpMachineCardHtml diretamente com o resultado.
//   - Novas funções: _marcarMaquinaAtendidaOS_ (marca vínculo OS_MAQUINAS como 'atendida' +
//     grava id_visita_resultado), _adicionarVisitaResultadoOS_ (acrescenta o ID da visita em
//     visit_ids_resultado_json da OS, sem duplicar em reenvio/retry).
//   - savePreventiva: quando body.os_id vem preenchido, chama as duas funções acima logo
//     após o autoCompletarTarefa já existente (que já convertia 'preventiva' em tipo de
//     tarefa 'auto:preventiva' e completava a tarefa do pipeline — só nunca recebia os_id
//     do PCF até este sprint). Ambas as chamadas são não-bloqueantes (try/catch).
// v42 — Sprint PCF v26→v27: Dados da Preventiva (auxiliar, hora início/fim)
//   - HEADERS.VISITAS: +Auxiliar, +Hora_Inicio, +Hora_Fim (colunas ao final,
//     retrocompatível — visitas antigas ficam com essas células vazias)
//   - saveVisit e savePreventiva: appendRow grava os 3 novos campos
//     (body.auxiliar / body.horaInicio / body.horaFim)
//   - getVisitsNormalized: parse de linha devolve auxiliar/horaInicio/horaFim
//     ao carregar uma visita existente (reabertura no PCF)
// v41 — Reparo de visitas órfãs (busca de cliente/máquina no PCF)
//   - Nova função: repararVisitasOrfas() — realinha VISITAS.Machine_ID inválido
//     para o ID canônico ativo em MAQUINAS, por correspondência cliente+série
//     (fallback cliente+TAG). Não apaga nada.
//   - doPost: novo case 'repararVisitasOrfas' (ação de escrita, disparada pelo PCM)
// v40 — Sessão E: Gantt de saturação de usuários
//   - HEADERS.PIPELINE_TAREFAS: +Usuario_Designado, +Iniciado_por
//   - registrarAbertura: grava Iniciado_por na primeira abertura da tarefa (não sobrescreve)
//   - Novas funções: designarUsuarioTarefa (admin define responsável), getSaturacaoUsuarios
//     (agrega fila por usuário designado + capacidade diária do regime de turno)
// v39 — Sessão D: tempo efetivo de execução (estimativa do usuário + dedução de turno)
//   - HEADERS.PIPELINE_TAREFAS: +Tempo_Efetivo_Min
//   - Novas funções: _calcularTempoEfetivoMin_, _horaParaData_, _aplicarTempoEfetivo_,
//     ajustarTempoEfetivoTarefa (correção manual do admin)
//   - updateTarefaStatus/autoCompletarTarefa: aceitam hora_inicio_estimada/hora_fim_estimada
//     e calculam Tempo_Efetivo_Min ao concluir, descontando turno/intervalos/dias não úteis
//   - saveFormulario: repassa as estimativas de horário para autoCompletarTarefa
// v38 — Sessão C: parametrização global de turno (ProperFlow)
//   - Nova aba PARAMETROS_TURNO (registro único, id='GLOBAL')
//   - Novas funções: getParametrosTurno (com defaults), saveParametrosTurno (upsert)
// v37 — Sessão B: checklists externos genéricos, sem hardcode
//   - HEADERS.PIPELINE_TAREFAS: +Form_URL
//   - autoCompletarTarefa: removido mapa fixo de tipos — qualquer trigger_type vira
//     'auto:<trigger_type>' automaticamente, sem precisar editar este arquivo
//   - instanciarPipeline: copia form_url do template para a tarefa instanciada
// v36 — Sessão A: perfil designado, data prevista e tempo estimado (ProperFlow)
//   - HEADERS.PIPELINE_TAREFAS: +Perfil_Responsavel, +Data_Prevista, +Tempo_Estimado_Min
//   - instanciarPipeline: copia perfil_responsavel/tempo_estimado_min do template
//   - updateTarefaStatus: aceita data_prevista opcional (definida pelo admin na OS)
...
const HEADERS = {
  MAQUINAS: [
    ...
    // v51 — estado atual do regime de trabalho (última visita que o enviou)
    'Regime_JSON'
  ],
  MODELOS: [
  ...
  VISITAS: [
    ...
    'OS_Numero',
    // v42 — Dados da Preventiva (auxiliar, hora início/fim)
    'Auxiliar','Hora_Inicio','Hora_Fim'
  ],
  PECAS_LOG: [
  ...
  ORDENS_SERVICO: [
    ...
    // v43 — Preventiva↔Pipeline: etiqueta interno/externo da OS (ao final, retrocompatível)
    'local_atendimento'
  ],
  TECH_ATTACHMENTS: [
...
HEADERS.PIPELINE_TAREFAS = [
  ...
  // v37 — Sessão B: URL do checklist/formulário externo vinculado à tarefa (ao final, retrocompatível)
  'Form_URL',
  // v39 — Sessão D: tempo efetivo calculado a partir da estimativa do usuário (ao final, retrocompatível)
  'Tempo_Efetivo_Min',
  // v40 — Sessão E: usuário designado e quem efetivamente iniciou a tarefa (ao final, retrocompatível)
  'Usuario_Designado','Iniciado_por'
];
  ...
  try {
    ...
    switch (action) {
      ...
      case 'getTiposOS':
        result = getTiposOS();
        break;
      case 'getSaturacaoUsuarios':
        result = getSaturacaoUsuarios();
        break;
      case 'verificarPinsDuplicados':
        result = verificarPinsDuplicados_();
        break;
      default:
  ...
  try {
    ...
    switch (action) {
      ...
      case 'getPipelineByOS':
        ...
        break;
      // v52 — read-passthrough por POST: os frontends chamam estas ações de LEITURA
      // via syncToGS/pgpPostJson (POST). Já existem no doGet; espelhadas aqui para
      // não caírem no default "Ação desconhecida". getMaquinasByOS/getEquipamentosByOS
      // leem body.id_os|body.numero_os (mesma assinatura que params no doGet).
      case 'getMaquinasByOS':
        result = getMaquinasByOS(body);
        break;
      case 'getEquipamentosByOS':
        result = getEquipamentosByOS(body);
        break;
      case 'getCatalogFull':
        result = getCatalogFull();
        break;
      case 'updateTarefaStatus':
        result = updateTarefaStatus(body.os_id, body.tarefa_id, body.status, body.usuario, body.observacao, body.data_prevista, body.hora_inicio_estimada, body.hora_fim_estimada);
        break;
      case 'getTemplates':
        result = getTemplates();
        break;
      ...
      case 'saveParametrosTurno':
        ...
        break;
      case 'ajustarTempoEfetivoTarefa':
        result = ajustarTempoEfetivoTarefa(body.os_id, body.tarefa_id, body.tempo_efetivo_min, body.usuario);
        break;
      case 'deleteTipoOS':
        result = deleteTipoOS(body.tipo_id);
        break;
      case 'designarUsuarioTarefa':
        result = designarUsuarioTarefa(body.os_id, body.tarefa_id, body.usuario_designado, body.usuario);
        break;
      case 'repararVisitasOrfas':
        result = repararVisitasOrfas();
        break;
      default:
        result = { status: 'error', error: 'Ação desconhecida: ' + action };
    }
...
var __savePreventivaMpData = null;
...
function savePreventiva(body) {
  ...
  const partsSheet = getOrCreateSheet('PECAS_LOG', HEADERS.PECAS_LOG);
  const machinePartsSheet = getOrCreateSheet('MACHINE_PARTS', HEADERS.MACHINE_PARTS);
  __savePreventivaMpData = machinePartsSheet.getDataRange().getValues();
  const pecasRows = [];
  let pecasTrocadas = 0;
  Object.entries(parts).forEach(([partId, ps]) => {
    ...
    if (acao === 'trocada') {
      ...
      // v51: passa dados extras para permitir upsert (criar linha) quando a
      // peça ainda não existe em MACHINE_PARTS para esta máquina.
      updateMachinePartValorMostrado(machineId, body.serial || '', body.tag || '', partId,
        safeVal(ps.valorMostrado), safeVal(ps.contador), now,
        { name: ps.name, na: ps.na, interval: ps.interval, lastChange: ps.lastChange });
    }
  });

  if (pecasRows.length) {
    const start = partsSheet.getLastRow() + 1;
    partsSheet.getRange(start, 1, pecasRows.length, pecasRows[0].length).setValues(pecasRows);
  }

  __savePreventivaMpData = null;
  ...
  // v43 — Preventiva↔Pipeline: grava o vínculo da visita de volta na OS e no OS_MAQUINAS
  // correspondente. Não-bloqueante: se falhar, a preventiva já salva acima não é revertida.
  if (body.os_id) {
    try { _marcarMaquinaAtendidaOS_(body.os_id, machineId, visitId); } catch(e) {}
    try { _adicionarVisitaResultadoOS_(body.os_id, visitId); } catch(e) {}
    try { _finalizarAtendimentoOS_(body.os_id, body.usuario_atual || body.tecnico_atual || body.responsavel || body.tech || 'sistema'); } catch(e) {}
  }
...
function getCurrentRefFromMachineParts(machineId, serial, tag, partId) {
  const sheet = getOrCreateSheet('MACHINE_PARTS', HEADERS.MACHINE_PARTS);
  const data = __savePreventivaMpData || sheet.getDataRange().getValues();
  if (data.length < 2) return '';
  const headers = data[0];
  const idxMid = headers.indexOf('Machine_ID');
  const idxSer = headers.indexOf('Serial');
...
// ═══════════════════════════════════════════════════════════════════════════
function updateMachinePartValorMostrado(machineId, serial, tag, partId, valorMostrado, contador, now, extra) {
  extra = extra || {};
  try {
    const sheet = getOrCreateSheet('MACHINE_PARTS', HEADERS.MACHINE_PARTS);
    const data = __savePreventivaMpData || sheet.getDataRange().getValues();
    ...
    // v51 — upsert: linha não existia (peça "conferida"/"N/A" numa máquina sem
    // MACHINE_PARTS ainda) — criar em vez de só logar e perder o dado.
    sheet.appendRow([
      machineId || '', serial || '', tag || '',
      partId,
      extra.name || partId,
      intOrBlank(extra.lastChange),
      intOrBlank(extra.interval),
      '', // Ref
      String(extra.na || '').toUpperCase() === 'SIM' ? 'SIM' : 'NÃO',
      '', // Ref_Anterior
      now || new Date().toISOString(),
      'SIM', 'PRODUCAO', '', '', now || new Date().toISOString(),
      safeVal(valorMostrado),
      safeVal(contador)
    ]);
  } catch(e) {
...
function ensureMachineFromVisit(body, machineId, visitDate, now) {
  ...
  // v51: coluna de estado atual do regime de trabalho
  const idxRegime = headers.indexOf('Regime_JSON');
  ...
  // v51: regime de trabalho — estado atual (última visita que o enviou)
  const regimeData = body.regimeData ? JSON.stringify(body.regimeData) : '';
  ...
  const applyRow = (rowIdx, preserveId) => {
    ...
    // v51: regime de trabalho — só grava se veio valor, nunca apaga o existente por omissão
    if (regimeData && idxRegime >= 0) sheet.getRange(rowIdx + 1, idxRegime + 1).setValue(regimeData);
...
function loginTecnico(body) {
  ...
  const modulosResult = resolverModulosUsuario_(tec.id);

  return {
    status: 'ok', valido: true,
    tecnico: {
      id: tec.id || '', nome: tec.nome || '', perfil: tec.perfil || '',
      modulos: modulosResult.modulos, is_admin: modulosResult.is_admin
    },
    os: osResult.os || []
  };
}

// v47 — Extraído de loginUsuarioHub (v32): resolve modulos_efetivos e is_admin
// de um usuário cruzando CAT_USUARIOS × CAT_PERFIS, para ser reutilizado tanto
// por loginTecnico (PIN simples) quanto por loginUsuarioHub (código morto).
// Mesma regra de negócio de sempre: modulos_efetivos = union(perfil.modulos_acesso,
// usuario.acesso_extra); is_admin do perfil libera todos os módulos existentes.
function resolverModulosUsuario_(idUsuario) {
  var idUsuarioStr = String(idUsuario || '').trim();
  var vazio = { modulos: [], is_admin: false };
  if (!idUsuarioStr) return vazio;
  ...
  var usuIdx = {}; usuHeaders.forEach(function(h, i) { usuIdx[h] = i; });

  var tec = null;
  for (var r = 1; r < usuData.length; r++) {
    if (String(usuData[r][usuIdx.id] || '').trim() === idUsuarioStr) {
  ...
  return { modulos: modulosEfetivos, is_admin: isAdmin };
}

// v46 — Auditoria manual: aponta usuários ativos que compartilham o mesmo
// pin_hash. Sem checagem automática disso, login por PIN simples (loginTecnico)
// fica ambíguo em caso de duplicidade — "vence" o primeiro match na planilha.
// Não bloqueia nada, é só um alerta para rodar manualmente quando quiser
// (?action=verificarPinsDuplicados&key=...).
function verificarPinsDuplicados_() {
  ensureSheetHeaders('CAT_USUARIOS', HEADERS.CAT_USUARIOS);
  const rows = getSheetData('CAT_USUARIOS');
  const porPinHash = {};

  rows.forEach(function(u) {
    if (!isActive_(u.ativo)) return;
    const pinHash = String(u.pin_hash || '').trim();
    if (!pinHash) return;
    if (!porPinHash[pinHash]) porPinHash[pinHash] = [];
    porPinHash[pinHash].push({ id: u.id || '', nome: u.nome || '' });
  });

  const duplicados = Object.keys(porPinHash)
    .filter(function(pinHash) { return porPinHash[pinHash].length > 1; })
    .map(function(pinHash) { return { pin_hash: pinHash, usuarios: porPinHash[pinHash] }; });

  return { status: 'ok', duplicados: duplicados };
}
  ...
  try {
    ...
    if (rowNum < 0) {
      ...
      const obj = {
        ...
        // v43 — Preventiva↔Pipeline: default 'externo' (caso histórico/majoritário até aqui)
        local_atendimento: (body.local_atendimento === 'interno' ? 'interno' : 'externo')
      };
    ...
    ALLOWED.forEach(function(k){
      if (Object.prototype.hasOwnProperty.call(body, k) && idx[k] >= 0) {
        ...
        } else if (k === 'local_atendimento') {
          v = (v === 'interno') ? 'interno' : 'externo';
        }
    ...
    // v50 — reconcilia OS_MAQUINAS com o novo conjunto de maquinas_vinculadas desta edição.
    // Não-bloqueante: erro aqui nunca deve reverter a OS já salva com sucesso acima
    // (mesmo padrão de _marcarMaquinaAtendidaOS_).
    if (idx.maquinas_vinculadas >= 0 && Object.prototype.hasOwnProperty.call(body, 'maquinas_vinculadas')) {
      try {
        const novasMaqs = Array.isArray(body.maquinas_vinculadas) ? body.maquinas_vinculadas : parseJsonArray_(body.maquinas_vinculadas);
        const novasMaqsSet = {};
        novasMaqs.forEach(function(mk){ novasMaqsSet[String(mk)] = true; });

        const idClienteAtual = body.id_cliente !== undefined ? body.id_cliente : osAtualizada.id_cliente;
        const numeroOsAtual = osAtualizada.numero_os;

        const maqSheet = getOrCreateSheet('OS_MAQUINAS', HEADERS.OS_MAQUINAS);
        const maqData = maqSheet.getDataRange().getValues();
        const maqHeaders = maqData[0] || HEADERS.OS_MAQUINAS;
        const maqIdx = {}; maqHeaders.forEach(function(h,i){ maqIdx[h]=i; });
        const nowMaq = new Date().toISOString();

        const ativasAtuais = {}; // machineKey -> rowNum (1-based) da linha ativa em OS_MAQUINAS
        for (let r = 1; r < maqData.length; r++) {
          if (String(maqData[r][maqIdx.id_os] || '') === String(id) && isActive_(maqData[r][maqIdx.Ativo])) {
            ativasAtuais[String(maqData[r][maqIdx.machineKey] || '')] = r + 1;
          }
        }

        // Vincula/reativa em OS_MAQUINAS as máquinas novas desta edição
        novasMaqs.forEach(function(mk){
          const key = String(mk);
          if (!ativasAtuais[key]) {
            _upsertOsMaquina_(id, numeroOsAtual, key, idClienteAtual, responsavel);
          }
        });

        // Desativa em OS_MAQUINAS as máquinas que saíram da seleção nesta edição
        Object.keys(ativasAtuais).forEach(function(key){
          if (!novasMaqsSet[key]) {
            const r = ativasAtuais[key];
            if (maqIdx.Ativo >= 0) maqSheet.getRange(r, maqIdx.Ativo + 1).setValue('NÃO');
            if (maqIdx.Updated_At >= 0) maqSheet.getRange(r, maqIdx.Updated_At + 1).setValue(nowMaq);
          }
        });
      } catch (eReconcileMaquinas) {
        Logger.log('saveOS: falha ao reconciliar OS_MAQUINAS na edição, id_os=' + id + ': ' + eReconcileMaquinas);
      }
    }
...
// v43 — Preventiva↔Pipeline: marca o vínculo OS_MAQUINAS correspondente como atendido,
// registrando qual visita (VISITAS.ID) resultou do atendimento. Não-bloqueante por padrão —
// erro aqui nunca deve reverter uma preventiva já salva com sucesso (chamar sempre em try/catch).
function _marcarMaquinaAtendidaOS_(idOs, machineKeyVal, visitId) {
  ...
  for (let r = 1; r < data.length; r++) {
    if (String(data[r][idx.id_os] || '') === String(idOs) && String(data[r][idx.machineKey] || '') === String(machineKeyVal)) {
      if (idx.status_atendimento >= 0) sheet.getRange(r + 1, idx.status_atendimento + 1).setValue('atendida');
      if (idx.id_visita_resultado >= 0) sheet.getRange(r + 1, idx.id_visita_resultado + 1).setValue(visitId);
      if (idx.Updated_At >= 0) sheet.getRange(r + 1, idx.Updated_At + 1).setValue(now);
      return;
    }
  }
...
// da OS, sem duplicar em caso de reenvio/retry (ex: fila offline).
function _adicionarVisitaResultadoOS_(idOs, visitId) {
  ensureOSHeaders_();
  const sheet = getOrCreateSheet('ORDENS_SERVICO', HEADERS.ORDENS_SERVICO);
  const data = sheet.getDataRange().getValues();
  const headers = data[0] || HEADERS.ORDENS_SERVICO;
  const idx = {}; headers.forEach(function(h,i){ idx[h]=i; });
  if (idx.id_os < 0 || idx.visit_ids_resultado_json < 0) return;
  const now = new Date().toISOString();
  for (let r = 1; r < data.length; r++) {
    if (String(data[r][idx.id_os] || '').trim() === String(idOs).trim()) {
      const atuais = parseJsonArray_(data[r][idx.visit_ids_resultado_json]);
      if (atuais.indexOf(visitId) === -1) {
        atuais.push(visitId);
        sheet.getRange(r + 1, idx.visit_ids_resultado_json + 1).setValue(stringifyArray_(atuais));
        if (idx.Updated_At >= 0) sheet.getRange(r + 1, idx.Updated_At + 1).setValue(now);
      }
      return;
    }
  }
  Logger.log('_adicionarVisitaResultadoOS_: OS não encontrada, id_os=' + idOs);
}

// v48 — Preventiva↔Pipeline: finaliza o atendimento da OS quando uma preventiva
// vinculada é registrada pelo PCF. Não chama saveOS e, portanto, não aciona a validação
// FECHAMENTO_INCOMPLETO usada no fechamento manual pelo PCM. Não-bloqueante por padrão:
// qualquer falha é logada e nunca deve reverter a preventiva salva com sucesso.
function _finalizarAtendimentoOS_(os_id, usuario) {
  try {
    const idOs = String(os_id || '').trim();
    if (!idOs) return;

    ensureOSHeaders_();
    const osSheet = getOrCreateSheet('ORDENS_SERVICO', HEADERS.ORDENS_SERVICO);
    const osData = osSheet.getDataRange().getValues();
    const osHeaders = osData[0] || HEADERS.ORDENS_SERVICO;
    const idxOs = {}; osHeaders.forEach(function(h,i){ idxOs[h]=i; });
    const idxIdOs = idxOs.id_os;
    const idxFim = idxOs.fim_atendimento;
    const idxStatus = idxOs.status;
    const idxUpdatedAt = idxOs.Updated_At;
    const idxUpdatedBy = idxOs.Updated_By;
    const now = new Date().toISOString();

    if (idxIdOs < 0 || idxFim < 0) {
      Logger.log('_finalizarAtendimentoOS_: colunas obrigatórias ausentes em ORDENS_SERVICO');
      return;
    }

    let rowOs = -1;
    for (let r = 1; r < osData.length; r++) {
      if (String(osData[r][idxIdOs] || '').trim() === idOs) {
        rowOs = r + 1;
        break;
      }
    }
    ...
    if (rowOs < 0) {
      Logger.log('_finalizarAtendimentoOS_: OS não encontrada, id_os=' + idOs);
      return;
    }

    osSheet.getRange(rowOs, idxFim + 1).setValue(now);
    if (idxUpdatedAt >= 0) osSheet.getRange(rowOs, idxUpdatedAt + 1).setValue(now);
    if (idxUpdatedBy >= 0) osSheet.getRange(rowOs, idxUpdatedBy + 1).setValue(usuario || 'sistema');

    ensureSheetHeaders('PIPELINE_TAREFAS', HEADERS.PIPELINE_TAREFAS);
    const tarefasSheet = getOrCreateSheet('PIPELINE_TAREFAS', HEADERS.PIPELINE_TAREFAS);
    const tarefasData = tarefasSheet.getDataRange().getValues();
    ...
    Logger.log('_finalizarAtendimentoOS_: erro não-bloqueante: ' + (e && e.message ? e.message : e));
  }
}
...
function getMaquinasByOS(params) {
  ...
  // v54: inclui máquinas inativas (Ativo=NÃO) na resolução — o vínculo da OS é um
  // fato histórico; uma máquina soft-deletada ainda deve aparecer na OS (marcada
  // como inativa) em vez de virar "Máquina não encontrada". Ativa tem precedência
  // sobre inativa quando ambas compartilham a mesma chave/ID.
  const maquinas = getSheetData('MAQUINAS');
  const maquinasPorKey = {};
  maquinas.forEach(function(m){
    const ativa = String(m['Ativo'] || 'SIM').trim().toUpperCase() !== 'NÃO';
    const key = machineKey(m['Cliente'] || '', m['Marca'] || '', m['Modelo'] || '', m['Série'] || '');
    if (!maquinasPorKey[key] || ativa) maquinasPorKey[key] = m;
    if (m['ID'] && (!maquinasPorKey[String(m['ID'])] || ativa)) maquinasPorKey[String(m['ID'])] = m;
  });
  ...
  vinculos.forEach(function(v){
    ...
    if (m) {
      // v43: normaliza pelo mesmo caminho de getMachinesByClient (rowToMachine + parts),
      // para que o PCF possa reaproveitar exatamente as mesmas funções de carregamento de
      // máquina que já usa no fluxo avulso de preventiva (pgpLoadMachine etc).
      const eq = rowToMachine(m);
      eq.parts = getMachinePartsById(eq.id, eq.serial, eq.tag);
      machines.push(Object.assign(eq, {
        inativa: String(m['Ativo'] || 'SIM').trim().toUpperCase() === 'NÃO', // v54
...
function rowToMachine(row) {
  return {
    ...
    type:      row['Tipo_Equip']   || '',
    obsOp:     row['Obs_Op']       || '',
    // v51 — aliases que o PCF (pgpLoadMachine) realmente lê ao restaurar a tela
    tipo:      row['Tipo_Equip']   || '',
    potencia:  row['Potência']     || '',
    regimeData: (() => {
      try { const v = row['Regime_JSON']; return v ? JSON.parse(v) : []; }
      catch(e) { return []; }
    })()
  };
}
...
function loginUsuarioHub(body) {
  ...
  // v44 — Segurança: rate-limit de tentativas de PIN. Sem isso, um PIN de 6
  // dígitos (1 milhão de combinações) podia ser tentado sem limite contra o
  // endpoint. Chave por id_usuario (fallback por prefixo do hash se vier sem
  // id, para não deixar tentativas anônimas sem controle nenhum).
  var LOGIN_MAX_TENTATIVAS = 5;
  var LOGIN_LOCKOUT_SEG    = 300; // 5 minutos
  var chaveRate = idUsuario || ('anon_' + pinHash.slice(0, 12));
  var cachePin  = CacheService.getScriptCache();
  var lockKey   = 'pin_lock_' + chaveRate;
  var failKey   = 'pin_fail_' + chaveRate;

  if (cachePin.get(lockKey)) {
    return { status: 'ok', valido: false, error: 'Muitas tentativas erradas. Aguarde alguns minutos e tente novamente.' };
  }
  ...
  if (!tec) {
    var tentativas = Number(cachePin.get(failKey) || 0) + 1;
    if (tentativas >= LOGIN_MAX_TENTATIVAS) {
      cachePin.put(lockKey, '1', LOGIN_LOCKOUT_SEG);
      cachePin.remove(failKey);
      return { status: 'ok', valido: false, error: 'Muitas tentativas erradas. Aguarde alguns minutos e tente novamente.' };
    }
    cachePin.put(failKey, String(tentativas), LOGIN_LOCKOUT_SEG);
    return { status: 'ok', valido: false, error: 'PIN inválido ou usuário inativo.' };
  }

  // Login certo — limpa contadores de tentativa deste usuário
  cachePin.remove(failKey);
  cachePin.remove(lockKey);
...
function verificarGoogleToken_(credential) {
  ...
  // v44 — Segurança: antes este código só decodificava o payload do JWT (base64)
  // sem checar a assinatura — qualquer string no formato certo, com aud/hd/exp
  // "corretos", passava. Agora validamos chamando o endpoint oficial do Google
  // (tokeninfo), que verifica a assinatura RSA no lado do Google e só devolve
  // o payload se o token realmente foi emitido por eles.
  var resp;
  try {
    ...
    throw new Error('Falha ao validar token com o Google: ' + e.message);
  }
  if (resp.getResponseCode() !== 200) {
    throw new Error('JWT inválido, expirado ou não assinado pelo Google');
  }
  var payload;
  try {
    payload = JSON.parse(resp.getContentText());
  } catch(e) {
    throw new Error('Resposta inválida do Google ao validar token');
  }
  if (payload.aud !== GOOGLE_CLIENT_ID_HUB) {
    throw new Error('JWT: audiência inválida');
  }
  var agora = Math.floor(Date.now() / 1000);
  if (!payload.exp || Number(payload.exp) < agora) {
...
function saveFormulario(body) {
  ...
  try {
    autoCompletarTarefa(body.os_id, body.tipo_formulario, {
      ...
      hora_inicio_estimada: body.hora_inicio_estimada || '',
      hora_fim_estimada: body.hora_fim_estimada || ''
    });
  } catch(e) {}
...
// ── getSaturacaoUsuarios ──────────────────────────────────────────────────────
// Agrega a carga de trabalho pendente/em_andamento por usuário designado, e
// calcula um indicador de saturação com base na capacidade diária do regime de
// trabalho global. Também retorna a lista de tarefas por usuário, para o Gantt.
function getSaturacaoUsuarios() {
  ensureSheetHeaders('PIPELINE_TAREFAS', HEADERS.PIPELINE_TAREFAS);
  ...
  // Capacidade diária (minutos) a partir do regime de turno global
  var paramResp = getParametrosTurno();
  var parametros = paramResp && paramResp.parametros;
  var capacidadeDiariaMin = 480; // fallback: 8h, caso não haja regime configurado
  if (parametros && parametros.turno_inicio && parametros.turno_fim) {
    var ini = _horaParaData_(new Date(2000, 0, 1), parametros.turno_inicio);
    var fim = _horaParaData_(new Date(2000, 0, 1), parametros.turno_fim);
    capacidadeDiariaMin = Math.max(0, (fim.getTime() - ini.getTime()) / 60000);
    var intervalos = [];
    try { intervalos = JSON.parse(parametros.intervalos_json || '[]'); } catch(e) {}
    intervalos.forEach(function(iv) {
      if (!iv || !iv.inicio || !iv.fim) return;
      var ivIni = _horaParaData_(new Date(2000, 0, 1), iv.inicio);
      var ivFim = _horaParaData_(new Date(2000, 0, 1), iv.fim);
      capacidadeDiariaMin -= Math.max(0, (ivFim.getTime() - ivIni.getTime()) / 60000);
    });
  }

  var porUsuario = {}; // usuario -> { total_tarefas, total_estimado_min, tarefas: [] }
  tarefas.forEach(function(t) {
    var usuario = String(t.Usuario_Designado || '').trim();
    if (!usuario) return; // tarefas sem designação não entram no cálculo de saturação
    if (!porUsuario[usuario]) {
      porUsuario[usuario] = { usuario: usuario, total_tarefas: 0, total_estimado_min: 0, tarefas: [] };
    }
    var estimado = parseInt(t.Tempo_Estimado_Min, 10) || 0;
    porUsuario[usuario].total_tarefas += 1;
    porUsuario[usuario].total_estimado_min += estimado;
    porUsuario[usuario].tarefas.push({
      os_id: t.OS_ID, tarefa_id: t.Tarefa_ID, nome: t.Nome, status: t.Status,
      tempo_estimado_min: estimado, data_prevista: t.Data_Prevista || ''
    });
  });

  var lista = Object.keys(porUsuario).map(function(k) {
    var u = porUsuario[k];
    u.dias_de_fila = capacidadeDiariaMin > 0 ? Math.round((u.total_estimado_min / capacidadeDiariaMin) * 10) / 10 : null;
    return u;
  });
  lista.sort(function(a, b) { return b.total_estimado_min - a.total_estimado_min; });

  return { status: 'ok', usuarios: lista, capacidade_diaria_min: capacidadeDiariaMin };
}

// ── Cálculo de tempo efetivo descontando turno/intervalos/dias não úteis ──
// Recebe dois timestamps ISO (início e fim estimados de execução) e o objeto de
// parâmetros de turno (mesmo formato retornado por getParametrosTurno().parametros).
// Retorna minutos efetivos (inteiro) ou null se os dados forem insuficientes/inválidos.
function _calcularTempoEfetivoMin_(inicioISO, fimISO, parametrosTurno) {
  if (!inicioISO || !fimISO || !parametrosTurno) return null;
  var inicio = new Date(inicioISO);
  var fim    = new Date(fimISO);
  if (isNaN(inicio.getTime()) || isNaN(fim.getTime()) || fim.getTime() <= inicio.getTime()) return null;

  var turnoInicioStr = parametrosTurno.turno_inicio || '08:00';
  var turnoFimStr    = parametrosTurno.turno_fim    || '18:00';
  var intervalos = [];
  try { intervalos = JSON.parse(parametrosTurno.intervalos_json || '[]'); } catch(e) {}
  var diasUteis = [1, 2, 3, 4, 5];
  try { diasUteis = JSON.parse(parametrosTurno.dias_uteis_json || '[1,2,3,4,5]'); } catch(e) {}

  var totalMin = 0;
  var cursor = new Date(inicio.getFullYear(), inicio.getMonth(), inicio.getDate());
  var diaFinal = new Date(fim.getFullYear(), fim.getMonth(), fim.getDate());

  while (cursor.getTime() <= diaFinal.getTime()) {
    var diaSemanaJs  = cursor.getDay();              // 0=domingo...6=sábado
    var diaSemanaISO = diaSemanaJs === 0 ? 7 : diaSemanaJs; // 1=segunda...7=domingo

    if (diasUteis.indexOf(diaSemanaISO) >= 0) {
      var turnoIniDia = _horaParaData_(cursor, turnoInicioStr);
      var turnoFimDia = _horaParaData_(cursor, turnoFimStr);
      var segIni = new Date(Math.max(turnoIniDia.getTime(), inicio.getTime()));
      var segFim = new Date(Math.min(turnoFimDia.getTime(), fim.getTime()));

      if (segFim.getTime() > segIni.getTime()) {
        var minutosDia = (segFim.getTime() - segIni.getTime()) / 60000;

        intervalos.forEach(function(iv) {
          if (!iv || !iv.inicio || !iv.fim) return;
          var ivIni = _horaParaData_(cursor, iv.inicio);
          var ivFim = _horaParaData_(cursor, iv.fim);
          var ovIni = new Date(Math.max(ivIni.getTime(), segIni.getTime()));
          var ovFim = new Date(Math.min(ivFim.getTime(), segFim.getTime()));
          if (ovFim.getTime() > ovIni.getTime()) {
            minutosDia -= (ovFim.getTime() - ovIni.getTime()) / 60000;
          }
        });

        totalMin += Math.max(0, minutosDia);
      }
    }
...
function _horaParaData_(diaBase, horaStr) {
  var partes = String(horaStr).split(':');
  return new Date(
    diaBase.getFullYear(), diaBase.getMonth(), diaBase.getDate(),
    parseInt(partes[0], 10) || 0, parseInt(partes[1], 10) || 0, 0
  );
}

// ── Aplica cálculo de tempo efetivo numa linha de PIPELINE_TAREFAS ────────
// Reaproveitada por updateTarefaStatus e autoCompletarTarefa para não duplicar
// a lógica de "determinar início/fim efetivo + calcular + gravar".
function _aplicarTempoEfetivo_(sheet, rowIndex, idxTempoEfetivo, dataAberturaAtual, horaInicioEstimada, horaFimEstimada, now) {
  if (idxTempoEfetivo < 0) return;
  var inicioEfetivo = horaInicioEstimada || dataAberturaAtual || null;
  if (!inicioEfetivo) return;
  var fimEfetivo = horaFimEstimada || now;
  var parametrosResp = getParametrosTurno();
  var tempoEfetivoMin = _calcularTempoEfetivoMin_(inicioEfetivo, fimEfetivo, parametrosResp && parametrosResp.parametros);
  if (tempoEfetivoMin !== null) {
    sheet.getRange(rowIndex, idxTempoEfetivo + 1).setValue(tempoEfetivoMin);
  }
}

// ── updateTarefaStatus ────────────────────────────────────────────────────
function updateTarefaStatus(os_id, tarefa_id, novoStatus, usuario, observacao, data_prevista, hora_inicio_estimada, hora_fim_estimada) {
  ...
  var idxAtv  = headers.indexOf('Ativo'); // v53
  var now     = new Date().toISOString();
  var found   = false;
  var statusAnterior = '';
  for (var i = 1; i < data.length; i++) {
    // v53: só a linha ATIVA da tarefa pode ser escrita — pipelines trocados/reinstanciados
    // deixam linhas ARQUIVADO/NÃO com o mesmo Tarefa_ID; sem isto a escrita caía na
    // linha arquivada e getPipelineByOS (que ignora arquivadas) mostrava a ativa vazia.
    ...
    if (String(data[i][idxOS] || '').trim()  === String(os_id).trim() &&
      ...
      if (novoStatus === 'completo') {
        _aplicarTempoEfetivo_(sheet, i + 1, idxTE, idxDatA >= 0 ? data[i][idxDatA] : null, hora_inicio_estimada, hora_fim_estimada, now);
      }
...
function autoCompletarTarefa(os_id, trigger_type, status_payload) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (novoStatus === 'completo' && idxDatC >= 0) sheet.getRange(i + 1, idxDatC + 1).setValue(now);
    if (novoStatus === 'completo') {
      _aplicarTempoEfetivo_(sheet, i + 1, idxTE, idxDatA >= 0 ? data[i][idxDatA] : null, status_payload.hora_inicio_estimada, status_payload.hora_fim_estimada, now);
    }
    if (idxUpd >= 0) sheet.getRange(i + 1, idxUpd + 1).setValue(now);
...
function registrarAbertura(os_id, tarefa_id, usuario) {
  ...
  var idxDatA = headers.indexOf('Data_abertura');
  var idxUpd  = headers.indexOf('Updated_At');
  var idxIniP = headers.indexOf('Iniciado_por');
  var idxAtv  = headers.indexOf('Ativo'); // v53
  var now     = new Date().toISOString();

  for (var i = 1; i < data.length; i++) {
    // v53: pula linhas arquivadas/soft-deletadas do mesmo Tarefa_ID (ver updateTarefaStatus)
    if (idxAtv >= 0 && ['ARQUIVADO','NÃO'].indexOf(String(data[i][idxAtv] || 'SIM').trim().toUpperCase()) >= 0) continue;
    if (String(data[i][idxOS]  || '').trim() !== String(os_id).trim())      continue;
    ...
    if (idxIniP >= 0 && !data[i][idxIniP] && usuario) sheet.getRange(i + 1, idxIniP + 1).setValue(usuario);
    if (idxUpd  >= 0) sheet.getRange(i + 1, idxUpd  + 1).setValue(now);
...
// ── ajustarTempoEfetivoTarefa ──────────────────────────────────────────────
// Sobrescrita manual (admin) do tempo efetivo calculado — não recalcula, só grava.
function ajustarTempoEfetivoTarefa(os_id, tarefa_id, tempo_efetivo_min, usuario) {
  if (!os_id || !tarefa_id || tempo_efetivo_min === undefined || tempo_efetivo_min === null) {
    return { status: 'error', error: 'os_id, tarefa_id e tempo_efetivo_min são obrigatórios' };
  }
  ensureSheetHeaders('PIPELINE_TAREFAS', HEADERS.PIPELINE_TAREFAS);
  var sheet   = getOrCreateSheet('PIPELINE_TAREFAS', HEADERS.PIPELINE_TAREFAS);
  var data    = sheet.getDataRange().getValues();
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (String(data[i][idxOS] || '').trim()  === String(os_id).trim() &&
        String(data[i][idxTar] || '').trim() === String(tarefa_id).trim()) {
      sheet.getRange(i + 1, idxTE + 1).setValue(parseInt(tempo_efetivo_min, 10));
      if (idxUpd >= 0) sheet.getRange(i + 1, idxUpd + 1).setValue(now);
      return { status: 'ok', os_id: os_id, tarefa_id: tarefa_id, tempo_efetivo_min: parseInt(tempo_efetivo_min, 10) };
    }
  }
  return { status: 'error', error: 'Tarefa não encontrada: ' + tarefa_id + ' na OS ' + os_id };
}

// ── designarUsuarioTarefa ────────────────────────────────────────────────────
// Admin designa manualmente qual usuário deve executar uma tarefa específica do
// pipeline. Não altera Status nem nenhum outro campo — só a designação.
function designarUsuarioTarefa(os_id, tarefa_id, usuario_designado, usuario) {
  if (!os_id || !tarefa_id) {
    return { status: 'error', error: 'os_id e tarefa_id são obrigatórios' };
  }
  ensureSheetHeaders('PIPELINE_TAREFAS', HEADERS.PIPELINE_TAREFAS);
  var sheet   = getOrCreateSheet('PIPELINE_TAREFAS', HEADERS.PIPELINE_TAREFAS);
  var data    = sheet.getDataRange().getValues();
  ...
  var idxOS   = headers.indexOf('OS_ID');
  var idxTar  = headers.indexOf('Tarefa_ID');
  var idxUD   = headers.indexOf('Usuario_Designado');
  var idxUpd  = headers.indexOf('Updated_At');
  var idxAtv  = headers.indexOf('Ativo'); // v53
  var now     = new Date().toISOString();
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (String(data[i][idxOS] || '').trim()  === String(os_id).trim() &&
        String(data[i][idxTar] || '').trim() === String(tarefa_id).trim()) {
      sheet.getRange(i + 1, idxUD + 1).setValue(usuario_designado || '');
      if (idxUpd >= 0) sheet.getRange(i + 1, idxUpd + 1).setValue(now);
      return { status: 'ok', os_id: os_id, tarefa_id: tarefa_id, usuario_designado: usuario_designado || '' };
    }
  }
  return { status: 'error', error: 'Tarefa não encontrada: ' + tarefa_id + ' na OS ' + os_id };
}
...
// ═══════════════════════════════════════════════════════════════════════════
// REPARO — repararVisitasOrfas
// Realinha VISITAS.Machine_ID quando o valor não corresponde a nenhum ID ativo
// em MAQUINAS, usando correspondência por cliente+série (fallback cliente+TAG).
// Não apaga nada — apenas corrige o vínculo. Rodar manualmente pelo admin (PCM).
// ═══════════════════════════════════════════════════════════════════════════
function repararVisitasOrfas() {
  const maquinas = getSheetDataActive('MAQUINAS');

  const porSerie = {};
  const porTag = {};
  maquinas.forEach(m => {
    const ck = clientKey(String(m['Cliente'] || ''));
    const serie = compactKey(String(m['Série'] || ''));
    const tag = compactKey(String(m['TAG'] || ''));
    const id = String(m['ID'] || '').trim();
    if (!id) return;
    if (serie) porSerie[ck + '|' + serie] = id;
    if (tag) porTag[ck + '|' + tag] = id;
  });

  const idsValidos = new Set(maquinas.map(m => String(m['ID'] || '').trim()).filter(Boolean));

  const sheet = SS.getSheetByName('VISITAS');
  if (!sheet) return { status: 'error', error: 'Aba VISITAS não encontrada.' };
  const data = sheet.getDataRange().getValues();
  const headers = data[0];
  const idxMachineId = headers.indexOf('Machine_ID');
  const idxCliente = headers.indexOf('Cliente');
  const idxSerie = headers.indexOf('Série');
  const idxTag = headers.indexOf('TAG');
  const idxAtivo = headers.indexOf('Ativo');
  if (idxMachineId < 0) return { status: 'error', error: 'Coluna Machine_ID não encontrada em VISITAS.' };
  ...
  for (let i = 1; i < data.length; i++) {
    const row = data[i];
    const machineId = String(row[idxMachineId] || '').trim();
    const ativo = String(row[idxAtivo] || 'SIM').toUpperCase();
    if (ativo === 'NÃO') continue;
    if (!machineId || idsValidos.has(machineId)) continue; // já válido ou vazio (fora do escopo)

    const ck = clientKey(String(row[idxCliente] || ''));
    const serie = compactKey(String(row[idxSerie] || ''));
    const tag = compactKey(String(row[idxTag] || ''));

    let novoId = (serie && porSerie[ck + '|' + serie]) || (tag && porTag[ck + '|' + tag]) || null;

    if (novoId) {
      sheet.getRange(i + 1, idxMachineId + 1).setValue(novoId);
      relinked.push({ linha: i + 1, visitaId: row[headers.indexOf('ID')], de: machineId, para: novoId });
    } else {
      unresolved.push({ linha: i + 1, visitaId: row[headers.indexOf('ID')], machineIdOrfao: machineId, cliente: row[idxCliente] });
    }
  }
  ...
  return {
    status: 'ok',
    ts: new Date().toISOString(),
    total_relinkados: relinked.length,
    total_nao_resolvidos: unresolved.length,
    relinked,
    unresolved
  };
}
